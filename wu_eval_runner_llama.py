r"""
Wu et al. (ISSTA 2023) — Eval Runner cho Llama 4 Scout

Song hanh voi wu_eval_runner.py (DeepSeek). File nay HARDCODE Llama 4 Scout —
...
def stage_generate(lang: str, n_patches: int, limit: int, workers: int, verbose: bool):
    ...
    print(f"[GENERATE llama-4-scout] lang={canonical_lang} total={len(cves)} "
...
def stage_validate(lang: str, n_patches: int, batch_size: int, prune_every_batch: bool,
    ...
    for r in range(1, n_patches + 1):
        ...
        print(f"\n=== RANK {r}/{n_patches} (llama-4-scout) — total={len(all_records)} "
...
# -- CLI ---------------------------------------------------------------------
def main():
    p = argparse.ArgumentParser(description="Wu et al. pipeline cho Llama 4 Scout qua OpenRouter")
