r"""
Wu et al. (ISSTA 2023) — Eval Runner cho Qwen3-14B

Song hanh voi wu_eval_runner.py (DeepSeek). File nay HARDCODE Qwen3-14B —
...
from wu_llm_runner_qwen import (
    generate_candidates_qwen, MODEL_ID, FILE_SLUG,
...
def _process_one_cve(cve: dict, canonical_lang: str, n_patches: int,
    ...
    t0 = time.time()
    try:
        raw_outputs = generate_candidates_qwen(
            cve_id=cve_id,
...
def stage_generate(lang: str, n_patches: int, limit: int, workers: int, verbose: bool):
    ...
    print(f"[GENERATE qwen3-14b] lang={canonical_lang} total={len(cves)} "
...
def stage_validate(lang: str, n_patches: int, batch_size: int, prune_every_batch: bool,
    ...
    for r in range(1, n_patches + 1):
        ...
        print(f"\n=== RANK {r}/{n_patches} (qwen3-14b) — total={len(all_records)} "
...
# -- CLI ---------------------------------------------------------------------
def main():
    p = argparse.ArgumentParser(description="Wu et al. pipeline cho Qwen3-14B qua OpenRouter")
