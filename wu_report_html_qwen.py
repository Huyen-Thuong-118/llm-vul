r"""
Wu et al. — HTML report generator cho Qwen3-14B (hardcoded).
...
from wu_llm_runner_qwen import MODEL_ID, FILE_SLUG
    ...
    for r in range(1, n_patches + 1):
        ...
        if st == "pass":
            ...
            cell_body = f'''<div style="font-size:13px;color:#94a3b8;margin-bottom:4px;">
                              CACH SUA (patch Qwen3-14B sinh ra):
                            </div>
...
# -- CLI ---------------------------------------------------------------------
def main():
    p = argparse.ArgumentParser(description="HTML report cho Qwen3-14B run")
    ...
    print(f"[LOAD] patches (qwen3-14b, n={args.n_patches})...")
