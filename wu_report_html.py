"""
wu_report_html.py — Build self-contained HTML report cho Wu et al. run tren PatchEval.

Doc:
  - wu_patches_<lang>_deepseek_r{1..N}.jsonl        (patch content per candidate)
  - patcheval/evaluation/evaluation_output/wu_<lang>_r{r}/
      ├── summary.json                              (fail breakdown per rank)
      └── logs/<cve>/{success,error}_output.log    (raw stdout Docker)
  - patcheval/datasets/patcheval_verified.json      (CVE metadata: desc, CWE, file, snippets)

Xuat:
  - wu_report_<lang>.html o goc PatchEval — 1 file HTML tu chua, mo bang browser.

Voi moi CVE hien thi:
  * metadata: description, CWE, file_path, dong bug
  * status per rank (pass / fail / khong chay)
  * neu PASS: patch diff (fix_patch tu jsonl) — CACH SUA
  * neu FAIL: phan loai theo taxonomy A1-D3 nguoi dung cung cap, kem log excerpt
              de manual review.

Chay:
  python wu_method\\wu_report_html.py --lang javascript --n-patches 3
  python wu_method\\wu_report_html.py --lang javascript --n-patches 5 --out custom_name.html
"""
...
# ── User's taxonomy (rut gon vao HTML) ────────────────────────────────────────
TAXONOMY = {
    "A1": ("A1. Loi bien dich",
           "Model sinh code sai cu phap / sai kieu / tham chieu ham khong ton tai. "
           "Giam manh khi co feedback tu trinh bien dich."),
    "A2": ("A2. Pha vo chuc nang hien co",
           "Patch bien dich duoc nhung lam hong hanh vi dung cua chuong trinh ngoai vung lo hong."),
    ...
    "B1": ("B1. Chon sai chien luoc sua",
           ...
           "51.4% patch that bai theo Why LLMs Fail."),
    "B4": ("B4. Chien luoc dung — chi tiet sai",
           "Huong tiep can dung nhung cai dat sai chi tiet."),
    "C7": ("C7. Khong xac nhan duoc",
           "Patch logic nhat quan voi ban sua cua lap trinh vien nhung khac cau truc "
           "hoac bo kiem thu / Docker co van de."),
    "INFRA": ("Infrastructure noise (KHONG phai reasoning fail)",
              "PATCH_APPLY_FAILED — diff khong ap dung duoc vao repo. Findings.md xac nhan "
              "day la 33-48% JS failures do _gen_build_unified_diff cu dung snippet tinh "
              "thay vi noi dung file thuc te."),
    "UNKNOWN": ("Chua phan loai — can review thu cong",
                "Log khong khop pattern nao. Xem log excerpt ben duoi."),
}


# ── Phan loai fail dua tren _error_type cua run_evaluation.py + heuristic log ──
def classify_failure(error_type: str, log_content: str) -> str:
    """
    error_type: apply_fail / compilation_fail / validation_fail (tu summary.json)
    log_content: noi dung error_output.log
    Tra ve code taxonomy (A1/A2/A3/B1/B4/C7/INFRA/UNKNOWN).
    """
    ...
    if error_type == "validation_fail":
        ...
        # Heuristic — PoC keyword thuong dung trong fix-run.sh cua PatchEval
        poc_hit = any(k in low for k in ("poc", "exploit", "vuln", "still vulnerable"))
        # Regression test keyword
        ...
        if reg_hit and poc_hit:
            return "B1"  # ca 2 fail → sai chien luoc
        # Khong tim thay tin hieu ro rang → co the la logic sai chi tiet
        return "B4"
...
def load_patches(lang: str, n_patches: int) -> dict:
    """cve_id -> {rank: fix_patch}"""
    out = {}
    for r in range(1, n_patches + 1):
        path = PATCHEVAL_ROOT / f"wu_patches_{lang.lower()}_deepseek_r{r}.jsonl"
        if not path.exists():
            continue
...
def _classify_error_from_log(log: str, language: str) -> str:
    """Reproduce logic cua run_evaluation.py::_error_type — de phan loai
    tu log content thay vi doc summary.json (bi ghi de qua cac batch)."""
    if not log:
        return "unknown"
...
def load_rank_results(lang: str, n_patches: int) -> dict:
    """
    Doc TRUC TIEP tu logs/<cve>/{success,error}_output.log thay vi summary.json,
    vi batch mode cua wu_eval_runner.py chay run_evaluation.py nhieu lan cung 1
    --output name → summary.json bi GHI DE moi batch, cuoi cung chi chua CVE
    cua batch cuoi. Log files thi accumulate binh thuong nen la source of truth.

    Returns:
      results[rank] = {
        "passed":    set[cve_id],
        "failed":    dict[cve_id -> error_type],   # apply_fail/compilation_fail/validation_fail
        "error_log": dict[cve_id -> raw error log content],
      }
    """
    canonical_lang = LANG_CANONICAL[lang.lower()]
    results = {}
    for r in range(1, n_patches + 1):
        out_dir = EVAL_OUT / f"wu_{lang.lower()}_r{r}"
...
# ── Aggregate: cross-rank status per CVE ──────────────────────────────────────
def build_cve_status(dataset: dict, patches: dict, results: dict, n_patches: int) -> list:
    """
    Return list of dicts, one per CVE:
      { cve, meta, status: {rank: "pass"|"fail"|"not_run"},
        fail_type: {rank: err_type}, tax_code: str (worst-case if all fail),
        first_pass_rank: int|None, patches: {rank: fix_patch},
        error_logs: {rank: raw_log} }
    """
    rows = []
    for cve_id in sorted(dataset.keys()):
        ...
        # Neu chua run rank nao va cung khong co patches → bo qua
        if all(s == "not_run" for s in status.values()) and cve_id not in patches:
            continue

        # Classify worst-case (nếu fail toan bo) — dung rank 1's log lam dai dien
        tax_code = None
        if first_pass is None:
            # all fail hoặc mix fail/not_run
            for r in range(1, n_patches + 1):
                if status[r] == "fail":
...
def render_diff(diff: str) -> str:
    """Render a unified diff with basic syntax highlighting."""
    if not diff:
...
def render_cve_detail(row: dict, n_patches: int) -> str:
    ...
    for r in range(1, n_patches + 1):
        ...
        if st == "pass":
            ...
            cell_body = f'''<div style="font-size:13px;color:#94a3b8;margin-bottom:4px;">
                              CACH SUA (patch DeepSeek sinh ra):
                            </div>
    ...
    # Reference fix (ground truth) for comparison
    ref_snippet = ""
...
<title>Wu et al. — {canonical} — Bao cao PatchEval</title>
<style>{CSS}</style>
</head>
<body>
<div class="container">
  ...
  <div class="meta">
    Sinh luc {now} · N candidates / CVE = {n_patches} · Model = DeepSeek V4-Flash · temperature = 0.8
  </div>
...
def main():
    ...
    p.add_argument("--n-patches", type=int, default=3,
                   help="So candidate / CVE (khop voi luc chay wu_eval_runner). Default 3.")
    p.add_argument("--out", type=str, default=None,
                   help="Path file HTML output. Default: wu_report_<lang>.html o goc PatchEval.")
    args = p.parse_args()

    canonical = LANG_CANONICAL[args.lang.lower()]
    print(f"[LOAD] dataset ({canonical})...")
    dataset = load_dataset(canonical)
    print(f"[LOAD] patches (n={args.n_patches})...")
    ...
    out_path = Path(args.out) if args.out else PATCHEVAL_ROOT / f"wu_report_{args.lang.lower()}.html"
