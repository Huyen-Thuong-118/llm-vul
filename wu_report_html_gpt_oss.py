r"""
Wu et al. — HTML report generator cho GPT-OSS 20B (hardcoded).
...
from wu_llm_runner_gpt_oss import MODEL_ID, FILE_SLUG
    ...
    for r in range(1, n_patches + 1):
        ...
        if st == "pass":
            ...
            cell_body = f'''<div style="font-size:13px;color:#94a3b8;margin-bottom:4px;">
                              CACH SUA (patch GPT-OSS 20B sinh ra):
                            </div>
...
# -- CLI ---------------------------------------------------------------------
def main():
    p = argparse.ArgumentParser(description="HTML report cho GPT-OSS 20B run")
    ...
    print(f"[LOAD] patches (gpt-oss-20b, n={args.n_patches})...")
